...
use crate::{title_to_slug, DocType, Document, ProjectState, Spike, SpikeOutcome};
...
pub fn handle_create(state: &ProjectState, args: &Value) -> Result<Value, HandlerError> {
    ...
    fs::write(&spike_path, &markdown).map_err(|e| HandlerError::StateLoadFailed(e.to_string()))?;
...
pub fn handle_complete(state: &ProjectState, args: &Value) -> Result<Value, HandlerError> {
    ...
    if let Some(p) = effective_path {
        let _ = crate::update_markdown_status(&state.home.docs_path.join(p), status_str);
    }
